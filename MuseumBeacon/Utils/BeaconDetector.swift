...
class BeaconDetector: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    private let stabilityThreshold = 6 //# of detections before beacon considered stable
    private let distanceFilter = 5 //unused
    private let headingFilter:Double = 3 // Update on every x degree change
    private let beaconHistoryChecks = 3 //# of accuracy readings that are kept - affects distances avg
    ...
    @Published var detectedBeacons: [BeaconInfo] = [] //for detectionTestingView
    ...
    func stopScanning() {
        ...
        // Optionally, manage VoiceOver as well
    ...
    func locationManager(_ manager: CLLocationManager, didRangeBeacons beacons: [CLBeacon], in region: CLBeaconRegion) {
        ...
        // Update detectedBeacons array for testing view
        for beaconUUID in sortedBeacons {
            if let average = beaconAverages[beaconUUID],
               let beaconInfo = BeaconSetup.beacons.first(where: { $0.id == beaconUUID }) {
                let beaconData = BeaconInfo(id: beaconUUID, name: beaconInfo.roomName, accuracy: average)
                updatedBeacons.append(beaconData)
            }
        }

        DispatchQueue.main.async {
            self.detectedBeacons = updatedBeacons
        }
